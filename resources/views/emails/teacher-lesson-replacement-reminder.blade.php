...
<head>
    ...
    <title>Rappel — demande de remplacement</title>
    <style>
        body { font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 640px; margin: 0 auto; padding: 20px; }
        .banner { background: #fef3c7; border: 1px solid #f59e0b; color: #92400e; padding: 14px 16px; border-radius: 8px; margin-bottom: 20px; font-size: 14px; }
...
<body>
    <div class="banner">
        <strong>Ceci est un e-mail automatique de relance.</strong>
        Vous n’avez pas encore répondu à une demande de remplacement
        @if($reminderTier === '48h')
            — le cours a lieu dans environ <strong>48 heures</strong>.
        @else
            — le cours a lieu dans environ <strong>24 heures</strong>.
        @endif
    </div>
    <div class="header">
        <h1 style="margin:0;font-size:20px;">Rappel — réponse attendue</h1>
        <p style="margin:8px 0 0 0;opacity:.95;">{{ $clubName }}</p>
    </div>
    <div class="content">
        <p>Bonjour {{ $replacementTeacherName }},</p>
        <p><strong>{{ $originalTeacherName }}</strong> attend toujours votre réponse pour le remplacement ci-dessous.</p>
        <p class="muted">Merci de vous connecter à votre <strong>espace enseignant</strong> pour <strong>accepter</strong> ou <strong>refuser</strong> sans délai.</p>
        ...
        @endif
        <p><strong>Cours concerné :</strong></p>
        <table>
            <thead>
                <tr>
                    <th>Date / heure</th>
        ...
        <p class="muted" style="margin-top:20px;">L’enseignant initial et le(s) responsable(s) du club sont mis en copie de ce message.</p>
    </div>
    <div class="footer">
        Message automatique de relance — Activibe / BookYourCoach
    </div>
</body>
</html>
