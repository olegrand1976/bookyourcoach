...
existingFiles.forEach(lang => {
    ...
    if (data) {
        ...
        const missingKeys = Array.from(allKeys).filter(key => !keySet.has(key));
...
if (fs.existsSync(nuxtConfigPath)) {
    ...
    SUPPORTED_LANGUAGES.forEach(lang => {
        if (configContent.includes(`code: '${lang}'`)) {
            ...
            console.log(`   ⚠️  Langue ${lang} non configurée dans nuxt.config.ts`);
        }
    });
} else {
...
// 5. Résumé final
console.log('\n📊 RÉSUMÉ:');
console.log(`   • Fichiers de langue: ${existingFiles.length}/${SUPPORTED_LANGUAGES.length}`);
console.log(`   • Clés de traduction totales: ${allKeys.size}`);
console.log(`   • Clés manquantes: ${totalMissingKeys}`);

if (existingFiles.length === SUPPORTED_LANGUAGES.length && totalMissingKeys === 0) {
