...
const { settings: globalSettings, loadSettings: loadGlobalSettings, saveSettings } = useSettings()
...
const loadSettings = async () => {
    try {
        ...
        console.log('🔧 [SETTINGS] Chargement des paramètres...')

        const response = await $api.get('/admin/settings/general')
        console.log('🔧 [SETTINGS] Réponse API:', response.data)

        if (response.data) {
            Object.assign(settings.value, response.data)
            console.log('🔧 [SETTINGS] Paramètres chargés:', settings.value)
        }
    } catch (error) {
...
onMounted(() => {
    loadSettings()
    loadStats()
    // Charger aussi les paramètres globaux pour le header
    loadGlobalSettings()
})
</script>
