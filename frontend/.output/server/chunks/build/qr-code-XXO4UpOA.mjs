...
const _sfc_main = {
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      var _a;
      _push(`<div${ssrRenderAttrs(mergeProps({ class: "min-h-screen bg-gray-50" }, _attrs))}><div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8"><div class="text-center mb-8"><h1 class="text-3xl font-bold text-gray-900 mb-2">QR Code du Club</h1><p class="text-gray-600"> Partagez ce QR code pour permettre aux enseignants et \xE9tudiants de rejoindre votre club </p></div><div class="bg-white rounded-xl shadow-lg p-6 mb-8"><div class="flex items-center space-x-4"><div class="bg-blue-100 p-3 rounded-lg"><svg class="w-8 h-8 text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4"></path></svg></div><div><h2 class="text-xl font-semibold text-gray-900">${ssrInterpolate(club.value.name)}</h2><p class="text-gray-600">${ssrInterpolate(club.value.email)}</p><p class="text-sm text-gray-500">${ssrInterpolate(club.value.address)}, ${ssrInterpolate(club.value.city)}</p></div></div></div><div class="bg-white rounded-xl shadow-lg p-8 mb-8"><div class="text-center"><h3 class="text-lg font-semibold text-gray-900 mb-4">Code QR du Club</h3><div class="flex justify-center mb-6">`);
      if (qrCodeData.value) {
        _push(`<div class="bg-white p-4 rounded-lg border-2 border-gray-200"><div class="w-64 h-64">${(_a = qrCodeData.value.qr_svg) != null ? _a : ""}</div></div>`);
      } else {
        _push(`<div class="w-64 h-64 bg-gray-100 rounded-lg flex items-center justify-center"><div class="text-center"><div class="animate-spin rounded-full h-12 w-12 border-b-2 border-blue-600 mx-auto mb-4"></div><p class="text-gray-600">G\xE9n\xE9ration du QR code...</p></div></div>`);
      }
      _push(`</div>`);
      if (qrCodeData.value) {
        _push(`<div class="space-y-2"><p class="text-sm text-gray-600"><span class="font-medium">Code:</span> ${ssrInterpolate(qrCodeData.value.qr_code)}</p><p class="text-sm text-gray-600"><span class="font-medium">G\xE9n\xE9r\xE9 le:</span> ${ssrInterpolate(formatDate(qrCodeData.value.generated_at))}</p></div>`);
      } else {
        _push(`<!---->`);
      }
      _push(`<div class="mt-6 flex flex-col sm:flex-row gap-4 justify-center"><button${ssrIncludeBooleanAttr(!qrCodeData.value) ? " disabled" : ""} class="bg-blue-600 text-white px-6 py-3 rounded-lg hover:bg-blue-700 transition-colors disabled:bg-gray-300 disabled:cursor-not-allowed flex items-center space-x-2"><svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path></svg><span>T\xE9l\xE9charger</span></button><button${ssrIncludeBooleanAttr(isRegenerating.value) ? " disabled" : ""} class="bg-green-600 text-white px-6 py-3 rounded-lg hover:bg-green-700 transition-colors disabled:bg-gray-300 disabled:cursor-not-allowed flex items-center space-x-2">`);
...
//# sourceMappingURL=qr-code-XXO4UpOA.mjs.map
