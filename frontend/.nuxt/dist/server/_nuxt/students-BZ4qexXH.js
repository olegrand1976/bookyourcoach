import { _ as _sfc_main$2 } from "./AddStudentModal-D-rIiqvu.js";
  ...
  setup(__props, { emit: __emit }) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      if (__props.isOpen) {
        ...
        if (mode.value === "qr") {
          _push(`<div class="space-y-4"><div class="text-center"><h4 class="text-lg font-medium text-gray-900 mb-2">Scanner le QR Code de l&#39;étudiant</h4><p class="text-gray-600 text-sm">Demandez à l&#39;étudiant d&#39;afficher son QR Code sur son écran</p></div><div class="bg-gray-50 rounded-lg p-6 text-center"><div class="mb-4"><svg class="w-16 h-16 mx-auto text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v1m6 11h2m-6 0h-2v4m0-11v3m0 0h.01M12 12h4.01M16 20h4M4 12h4m12 0h.01M5 8h2a1 1 0 001-1V5a1 1 0 00-1-1H5a1 1 0 00-1 1v2a1 1 0 001 1zm12 0h2a1 1 0 001-1V5a1 1 0 00-1-1h-2a1 1 0 00-1 1v2a1 1 0 001 1zM5 20h2a1 1 0 001-1v-2a1 1 0 00-1-1H5a1 1 0 00-1 1v2a1 1 0 001 1z"></path></svg></div><p class="text-gray-600 mb-4">Fonctionnalité de scan QR Code à implémenter</p><button class="bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 transition-colors"> Simuler le scan (Test) </button></div></div>`);
        } else {
          _push(`<!---->`);
        }
        if (mode.value === "search") {
          _push(`<div class="space-y-4"><div><label class="block text-sm font-medium text-gray-700 mb-2"> Rechercher un étudiant </label><div class="relative"><input${ssrRenderAttr("value", searchQuery.value)} type="text" placeholder="Nom, email ou téléphone..." class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">`);
          ...
          if (searchResults.value.length > 0) {
            _push(`<div class="space-y-2"><h4 class="text-sm font-medium text-gray-700">Résultats de recherche :</h4><div class="max-h-60 overflow-y-auto space-y-2"><!--[-->`);
            ssrRenderList(searchResults.value, (user) => {
              ...
              _push(`</div><div class="text-sm text-gray-500">${ssrInterpolate(user.role === "student" ? "Étudiant" : user.role)}</div></div></div>`);
            });
            _push(`<!--]--></div></div>`);
          } else {
            _push(`<!---->`);
          ...
          if (searchQuery.value && !isSearching.value && searchResults.value.length === 0) {
            _push(`<div class="text-center py-4"><p class="text-gray-500">Aucun étudiant trouvé</p></div>`);
          } else {
            _push(`<!---->`);
          }
          _push(`</div>`);
        ...
        if (selectedUser.value) {
          _push(`<div class="mt-6 p-4 bg-blue-50 rounded-lg"><h4 class="font-medium text-blue-900 mb-2">Étudiant sélectionné :</h4><div class="flex items-center justify-between"><div><p class="font-medium text-blue-900">${ssrInterpolate(selectedUser.value.name)}</p><p class="text-sm text-blue-700">${ssrInterpolate(selectedUser.value.email)}</p></div><button class="text-blue-600 hover:text-blue-800"><svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg></button></div></div>`);
        } else {
          _push(`<!---->`);
        }
      ...
      if (filteredStudents.value.length === 0) {
        ...
        ssrRenderList(filteredStudents.value, (student) => {
          ...
          if (student.medical_info) {
            _push(`<div class="mt-1 text-sm text-amber-600"><span class="font-medium">⚠️ Infos médicales:</span> ${ssrInterpolate(student.medical_info.substring(0, 80))}${ssrInterpolate(student.medical_info.length > 80 ? "..." : "")}</div>`);
          } else {
            _push(`<!---->`);
          }
...
//# sourceMappingURL=students-BZ4qexXH.js.map
