...
const _sfc_main = {
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      var _a, _b;
      _push(`<div${ssrRenderAttrs(mergeProps({ class: "min-h-screen bg-gray-50" }, _attrs))}><div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8"><div class="mb-8"><h1 class="text-3xl font-bold text-gray-900">Mon QR Code</h1><p class="mt-2 text-gray-600"> Présentez ce QR code aux clubs pour vous ajouter rapidement </p></div><div class="bg-white rounded-xl shadow-lg overflow-hidden"><div class="px-6 py-4 border-b border-gray-200 bg-gradient-to-r from-blue-50 to-indigo-50"><div class="flex items-center justify-between"><div class="flex items-center space-x-3"><div class="bg-blue-100 p-2 rounded-lg"><svg class="w-6 h-6 text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v1m6 11h2m-6 0h-2v4m0-11v3m0 0h.01M12 12h4.01M16 20h4M4 12h4m12 0h.01M5 8h2a1 1 0 001-1V5a1 1 0 00-1-1H5a1 1 0 00-1 1v2a1 1 0 001 1zm12 0h2a1 1 0 001-1V5a1 1 0 00-1-1h-2a1 1 0 00-1 1v2a1 1 0 001 1zM5 20h2a1 1 0 001-1v-2a1 1 0 00-1-1H5a1 1 0 00-1 1v2a1 1 0 001 1z"></path></svg></div><div><h3 class="text-lg font-semibold text-gray-900">Code QR Personnel</h3><p class="text-sm text-gray-600">${ssrInterpolate((_a = user.value) == null ? void 0 : _a.name)} - ${ssrInterpolate((_b = user.value) == null ? void 0 : _b.email)}</p></div></div><button${ssrIncludeBooleanAttr(loading.value) ? " disabled" : ""} class="bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 disabled:opacity-50 transition-colors text-sm font-medium">${ssrInterpolate(loading.value ? "Génération..." : "Régénérer")}</button></div></div><div class="p-8">`);
      if (qrData.value) {
        _push(`<div class="text-center"><div class="inline-block p-4 bg-white border-2 border-gray-200 rounded-lg shadow-sm"><div class="mx-auto">${qrData.value.qr_svg ?? ""}</div></div><div class="mt-6 space-y-2"><p class="text-sm text-gray-600">Code QR:</p><p class="font-mono text-sm bg-gray-100 px-3 py-2 rounded-lg inline-block">${ssrInterpolate(qrData.value.qr_code)}</p></div><div class="mt-4 text-xs text-gray-500"> Généré le ${ssrInterpolate(formatDate(qrData.value.generated_at))}</div></div>`);
      } else {
...
//# sourceMappingURL=qr-code-roGIwXAk.js.map
