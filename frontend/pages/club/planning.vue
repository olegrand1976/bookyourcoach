  ...
  <div class="min-h-screen bg-gray-50 p-8">
      ...
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
        <div class="flex items-center justify-between">
          ...
          <div class="flex items-center space-x-3">
            ...
            <button @click="showCreateLessonModal = true"
      ...
      <div class="max-w-7xl mx-auto py-4">
        <div class="flex items-center justify-between flex-wrap gap-4">
          ...
          <div class="flex items-center space-x-4">
            <button @click="viewMode === 'week' ? previousWeek() : previousDay()"
            ...
            <button @click="viewMode === 'week' ? nextWeek() : nextDay()"
              class="p-2 text-gray-400 hover:text-gray-600 transition-colors">
              <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
          ...
          <div class="bg-white border border-gray-200 rounded-lg p-4 mb-4">
            <div class="flex items-center justify-between mb-3">
              ...
              <button @click="showAddSlotModal = true" class="text-blue-600 hover:text-blue-800 text-sm">+
                Ajouter</button>
            </div>
            ...
            <div v-else class="divide-y divide-gray-100">
              <div v-for="slot in availableSlots" :key="slot.id" class="py-3 grid grid-cols-12 gap-3 items-center">
                ...
                <div class="col-span-2">
                  <select v-if="slot.editing" v-model.number="editBuffer[slot.id].day_of_week"
                ...
                <div class="col-span-2 flex items-center gap-1">
                  <template v-if="slot.editing">
                    <input v-model="editBuffer[slot.id].start_time" type="time"
                      class="border border-gray-300 rounded px-2 py-1 text-sm">
                    <span class="text-gray-400 text-xs">→</span>
                    <input v-model="editBuffer[slot.id].end_time" type="time"
                      class="border border-gray-300 rounded px-2 py-1 text-sm">
                  </template>
                ...
                <div class="col-span-4">
                  <template v-if="slot.editing">
                    <select v-model="editBuffer[slot.id].discipline_id"
                      class="w-full border border-gray-300 rounded px-2 py-1 text-sm">
                      <option :value="null">(Aucune)</option>
                      <option v-for="d in availableDisciplines" :key="d.id" :value="d.id">{{ d.name }}</option>
                    </select>
                  </template>
                  <div v-else class="text-sm text-gray-800 truncate"
                    :title="(availableDisciplines.find(d => d.id === slot.discipline_id)?.name) || '—'">
                ...
                <div class="col-span-1 text-center">
                  <template v-if="slot.editing">
                    <input v-model.number="editBuffer[slot.id].max_capacity" type="number" min="1"
                      class="w-full border border-gray-300 rounded px-2 py-1 text-sm">
                  </template>
                ...
                <div class="col-span-3 flex justify-end gap-1">
                  <template v-if="slot.editing">
                    <button @click="saveSlotEdit(slot)"
                      class="text-emerald-600 hover:text-emerald-800 text-xs px-2 py-1 rounded">✓</button>
                    <button @click="cancelSlotEdit(slot)"
                      class="text-gray-500 hover:text-gray-700 text-xs px-2 py-1 rounded">✕</button>
                  </template>
                  <template v-else>
                    <button @click="openAddSlotModal(slot)"
                    ...
                    <button @click="confirmDeleteSlot(slot)"
      ...
      <div class="bg-white rounded-lg shadow-lg overflow-hidden border border-gray-200">
        <!-- En-têtes des jours -->
        <div :class="viewMode === 'week' ? 'grid grid-cols-8' : 'flex'"
          class="bg-gradient-to-b from-gray-50 to-gray-100 border-b-2 border-gray-300">
          <div
            class="p-4 text-center text-xs font-semibold text-gray-500 uppercase tracking-wide flex items-center justify-center"
            :class="viewMode === 'week' ? '' : 'w-20 flex-shrink-0'">
            Horaires
          </div>
          ...
          <div class="relative" :style="{ height: `${hourRanges.length * 60}px` }">
            ...
            <div v-for="(hour, index) in hourRanges" :key="`hour-${hour}`"
              ...
              <div :class="viewMode === 'week' ? 'grid grid-cols-8' : 'flex'" class="h-full">
                ...
                <!-- Colonnes des jours -->
                <div v-for="(day, dayIndex) in displayDays" :key="`grid-${day.date}-${hour}`" :class="[
                  ...
                  <div v-if="!isSlotFull(day.date, hour)"
            ...
            <div v-for="(day, dayIndex) in displayDays" :key="`openslots-${day.date}`"
              class="absolute top-0 pointer-events-none" :style="{
              ...
              <div v-for="slot in getOpenSlotsForDay(day.date)" :key="slot.id"
                ...
                <div class="absolute inset-0 flex">
                  <div v-for="i in slot.max_capacity" :key="`slot-${slot.id}-${i}`" :class="[
                ...
                <div :class="[
                  ...
                  <span class="mx-1">•</span>
                  <span>{{ getUsedSlotsForDateTime(day.date, slot.start_time, slot) >= slot.max_capacity ? 'COMPLET' :
                    'Ouvert' }}</span>
                </div>
              </div>
            </div>
            ...
            <div v-for="(day, dayIndex) in displayDays" :key="`lessons-${day.date}`"
              ...
              <div v-for="lesson in getLessonsForDayWithColumns(day.date)" :key="lesson.id"
                ...
                <div v-if="lesson.student_name && lesson.totalColumns === 1"
                  class="text-[11px] opacity-75 truncate mt-0.5">
                  👤 {{ lesson.student_name }}
                </div>
              </div>
            </div>
      ...
      <div class="bg-white rounded-lg p-6 w-full max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <div class="space-y-4">
          ...
          <div v-if="clubActivities.length > 1">
            ...
            <select v-model="slotForm.activity_type_id" @change="onActivityChange"
              class="w-full border border-gray-300 rounded-lg px-3 py-2">
          ...
          <div>
            ...
            <select v-model="slotForm.discipline_id" :disabled="filteredDisciplinesForSlot.length === 0"
              class="w-full border border-gray-300 rounded-lg px-3 py-2"
              :class="{ 'bg-gray-100 cursor-not-allowed': filteredDisciplinesForSlot.length === 0 }">
              <option value="">Sélectionner...</option>
              <option v-for="discipline in filteredDisciplinesForSlot" :key="discipline.id" :value="discipline.id">{{
          ...
          <div>
            ...
            <input v-model.number="slotForm.max_capacity" type="number" min="1" max="10"
              class="w-full border border-gray-300 rounded-lg px-3 py-2">
          </div>
          <div class="grid grid-cols-2 gap-4">
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-2">Durée du cours (min)</label>
              <input v-model.number="slotForm.duration" type="number" min="15" step="5"
                class="w-full border border-gray-300 rounded-lg px-3 py-2">
            </div>
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-2">Prix (€)</label>
              <input v-model.number="slotForm.price" type="number" min="0" step="0.01"
                class="w-full border border-gray-300 rounded-lg px-3 py-2">
            </div>
          </div>
        </div>
        <div class="flex items-center justify-end space-x-3 mt-6">
          ...
          <button @click="saveSlot"
            class="bg-blue-600 text-white px-6 py-2 rounded-lg hover:bg-blue-700">Enregistrer</button>
        </div>
      </div>
    </div>

    <!-- Modale d'édition de créneau -->
    <div v-if="showEditSlotModal"
      ...
      <div class="bg-white rounded-lg p-6 w-full max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <div class="flex items-center justify-end space-x-3 mt-6">
          ...
          <button @click="updateSlot"
            class="bg-blue-600 text-white px-6 py-2 rounded-lg hover:bg-blue-700">Modifier</button>
        </div>
      </div>
    </div>

    <!-- Modal Créer un cours -->
    <div v-if="showCreateLessonModal"
      ...
      <div class="bg-white rounded-lg p-6 w-full max-w-2xl my-8">
        ...
        <div class="grid grid-cols-2 gap-4">
          <div>
            ...
            <input v-model="lessonForm.date" type="date"
          ...
          <div>
            ...
            <select v-model="lessonForm.teacherId"
              class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
          ...
          <div class="col-span-2">
            ...
            <textarea v-model="lessonForm.notes" rows="3" placeholder="Notes complémentaires..."
        ...
        <div class="flex items-center justify-end space-x-3 mt-6">
          ...
          <button @click="createLesson" :disabled="!lessonForm.date || !lessonForm.time || !lessonForm.teacherId"
